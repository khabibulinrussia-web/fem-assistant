...
export function runCalculation(a: Record<string, any>): Record<string, any> {
  ...
  const payrollMonthly: number[] = Array.from({ length: MONTHS }, (_, m) => {
    const basePayroll = staffTotal * Math.pow(1 + INF, m);
    const taxes = staffTotal * payrollTaxRate * Math.pow(1 + INF, m);
    return Math.round((basePayroll + taxes + insuranceFixed / 12 + insurance1pct / 12) * 100) / 100;
  });

  // Total fixed = fixed_costs + payroll
  const totalFixed: number[] = fixedCosts.map((fc, m) =>
    Math.round((fc + payrollMonthly[m]) * 100) / 100
  );
  ...
  // === Break-even ===
  const avgPayroll = payrollMonthly.reduce((a, b) => a + b, 0) / MONTHS;
  const avgFixed = fixedSum + avgPayroll + acquiring.reduce((a, b) => a + b, 0) / MONTHS;
  ...
  return {
    ...
    pnl: {
      ...
      salary: payrollMonthly.map((_, m) => Math.round(staffTotal * Math.pow(1 + INF, m) * 100) / 100),
      payroll_taxes: payrollMonthly.map((pm, m) => {
        const base = staffTotal * Math.pow(1 + INF, m);
        const taxes = staffTotal * payrollTaxRate * Math.pow(1 + INF, m);
        return Math.round((pm - base - taxes) * 100) / 100;
      }),
