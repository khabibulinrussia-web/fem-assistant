...
class Assumptions(BaseModel):
    ...
    # Доходы
    target_revenue_per_day: float = 0  # Целевая выручка в день
    avg_check: float = 0  # Средний чек
    orders_per_day: float = 0  # Заказов в день
    ramp_month_1: float = 1.0  # Ramp-up мес 1
    ramp_month_2: float = 1.0  # Ramp-up мес 2
    ramp_month_3: float = 1.0  # Ramp-up мес 3+

    # Продукты
    products: Optional[List[dict]] = None  # Массив продуктов со скидками

    # Расходы
    cost_per_order: float = 0  # Себестоимость заказа
    payroll: float = 0  # ФОТ в месяц
    ...
    # Инвестиции
    reg_ip: float = 0  # Регистрация ИП
    equipment: float = 0  # Оборудование
